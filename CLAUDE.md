...
1. **Classify cascade-mode BEFORE picking a tool.** Litmus: *can one rule decide every instance?* Yes → (a) deterministic rule/fabric fix [or (b) substrate-repair if the defect clusters on bad fabric]. Judgment-per-instance AND wrong-call costly (over-merge = red line) → (c) **pipeline** (rule 2). Single instance → never fly-swat; find the class. **Second tell:** if another layer/corpus segments it right, it's almost always a missing rule (a), not a pipeline (c). State mode + 1-line reason, proceed. → [[memories/operational/feedback_no_fly_swatting.md|feedback_no_fly_swatting.md]]

2. **Multi-agent fan-out → encode as a `Workflow` script, NOT hand-spawned `Agent` calls.** Hand-spawning ≥2 coordinated agents loses determinism, parallelism, and resumability, and dies at compaction. Canonical v2-spray: `pipeline(candidates → parallel adjudicate → ≥2 parallel audits [over-merge + atomicity]) → gate → HALT` (returns survivors; deploy is a separate gated decision). Mechanical fan-out ONLY — never workflow a methodology/judgment call, never fold deploy into the script. **Every sub-agent spec naming a file / module / function / lemma / feature MUST require pasted verification receipts** (`ls` / `wc -c` / `Grep` / TF-query output) in its StructuredOutput. **Design-space: ≥2 viable variants → simulate ALL against the anchor cases in parallel, audit only survivors** — never serial "v(N+1) after audit feedback" iteration. → [[memories/operational/feedback_subagent_specs_require_receipts.md|feedback_subagent_specs_require_receipts.md]], [[memories/operational/feedback_code_path_diagnoses_require_running_the_code.md|feedback_code_path_diagnoses_require_running_the_code.md]]

3. **Compaction-resume = CONTINUE the in-flight cascade.** Enumerate state from files/git/gates; never recall a pre-compaction comparative number without re-running the gate. → [[memories/operational/feedback_verify_deploy_state_never_assert.md|feedback_verify_deploy_state_never_assert.md]]

4. **Data/treebank-first for OWN reasoning.** "Cannot / impossible / needs LLM" is the STOP signal — inventory every dataset Stan gave you before asserting it. → [[memories/operational/feedback_mechanical_first_for_own_review.md|feedback_mechanical_first_for_own_review.md]]

5. **Consult prior work before building — three granularities.** (a) Cross-corpus: "how did Tanakh/GNT/BoFM solve this?" — port, don't re-invent. (b) Same-file: before proposing a new `_is_*`/`_has_*`/`_detect_*`/`_check_*` helper, `Grep` the file for an analogous one and cite why a new one is needed. (c) **File the answer back.** When a cross-corpus consult produces a durable answer, write it to `2-evidence/<topic>.md` **in the same turn** and index it in [[00-start-here.md]] — an answer that dies in chat is re-derived by grep on every future question, which is the tax (a) exists to avoid. → [[memories/operational/feedback_check_prior_corpora.md|feedback_check_prior_corpora.md]]

6. **Verify empirically — inspection ≠ verification.** Source-parity (vs original USFM/TF/source) is the correctness gate; index-parity (vs HEAD) is regression-only. Run BOTH tactical (does this break?) and strategic (is this the right design?) audits before any pivot. Any **new mechanism / integration / env-flag / guard structure** triggers the §7.3 audit gate BEFORE code (substrate *consumption* is exempt; building a new integration shape over it is not). Any code-touching edit that **cites canon** ([[1-method/framework.md|framework.md]] / `§2.x` / `binding-rules-*`) requires a fresh in-turn `Read` of cited-range + 30 lines downstream AND a verbatim firewall quote in the proposal. **A detector is itself a claim — calibrate before sweeping.** *(Promoted 2026-08-09 from [[4-process/lessons.md|lessons.md]]; new, displaces nothing.)* No checker may report until a **known-good it must find** and a **known-bad it must not** are asserted **in the script** and pass; a tool that fails calibration refuses to report. Four detectors were miscalibrated in one day — a wikilink checker reporting 0 while the resolver flagged links, a density metric that penalised the comparator's style and inverted the conclusion, a `\n`/`\f` regex "finding" 28,829 markers that were escape sequences, and an audit dispatched to enforce this rule reproducing that third one. **When a scan says most of the corpus is broken, suspect the scan.** Reference implementation: `5-machinery/scripts/decision_log.py --calibrate`. Pre-audit self-check: every cited `file:line` Read this turn, every governing helper observed firing via probe (not inferred), every analogous helper Grep-enumerated. → [[memories/operational/feedback_never_skip_audit_gate.md|feedback_never_skip_audit_gate.md]], [[memories/operational/feedback_conformance_is_not_correctness.md|feedback_conformance_is_not_correctness.md]], [[memories/operational/feedback_canon_citation_requires_verbatim_read.md|feedback_canon_citation_requires_verbatim_read.md]]

7. **ANY claim about an artifact — code path, file contents, repo state, prior work, a live site — binds to a fresh in-turn `Read`/`Grep`/`Run`, never recall.** *(Scope generalised 2026-08-09, promoted from [[4-process/lessons.md|lessons.md]]; displaces the former code-path-only wording, which let every non-code artifact claim through.)* One class, four instances all committed in a single session: measuring HTML renderers and calling them rule code; proposing an experiment the canon records as already run and failed; scoring a component absent when 62 YAML specs implement it; asserting a dated verification about another workspace that had flipped hours earlier. **Litmus: if a sentence asserts what an artifact contains, does, or lacks — open it this turn.** Negative claims ("X does not exist", "was never built") are the highest bar. Before naming any `file:line` as the bug / cause / gate / integration locus: Read it this turn; Grep the file for analogous `_is_*`/`_has_*`/`_detect_*` helpers before designing a new one; for control-flow claims ("rule R fires here") run a probe that *observes* the firing, never infer from a static read. STOP signal: *"I think rule R at line N."* Live-input-path pins + full incident record → [[memories/operational/feedback_code_path_diagnoses_require_running_the_code.md|feedback_code_path_diagnoses_require_running_the_code.md]].

8. **Stan-flagged verse → Read-canon-then-situate, NEVER fix-then-ship.** First three tool calls, in order: (a) `Read` the verse ±2 from the live source (`v2-adjudicated/` override → `v2/` baseline); (b) `Read` [[1-method/framework.md#§2.1 The bidirectional test (primary criterion)|framework.md §2.1]] AND §2.2 in full including the Registry-discipline firewall (~lines 113-117); (c) write a **bidirectional-test walkthrough** in chat — forward-closure + backward-containment per side, with the §2.1 allowance / §2.2 marker / firewall clause quoted verbatim. Only then does rule-design / `Edit` / audit-dispatch happen. Execute-don't-ask does NOT license skipping (a)/(b)/(c) — the situate-walkthrough IS the executing here. → [[memories/operational/feedback_canon_citation_requires_verbatim_read.md|feedback_canon_citation_requires_verbatim_read.md]], [[memories/operational/feedback_scrutinize_stan_instincts.md|feedback_scrutinize_stan_instincts.md]]

---

## Closed routes / banked-gold / settled tactical — DO NOT re-litigate

Headlines only; full record + warrants at [[memories/operational/_north_star.md|_north_star.md]] (mandatory orientation read). If something here looks reopenable, it isn't — bring it to Stan with evidence before touching.
...
| File / dir | Trigger |
|---|---|
| `repos/atu-method/1-method/framework.md` | Any methodology / rule-design / canon-touching question |
| `repos/atu-method/1-method/binding-rules-hebrew.md` | Hebrew binding-rule catalog (B1-B14) work |
| `repos/atu-method/3-implementation/toolset-architecture.md` | Pipeline implementation (v0→v3 stages) |
| `repos/atu-method/3-implementation/apparatus.md` | English-layer work, swap-system, 4-layer integrity |
...
Mechanical-first per `1-method/framework.md` §3. **Live state per reader = `2-evidence/deployment-status.md` — single source of truth; never infer it from a stale per-repo doc.** Stages, ceiling figures, and the three past-ceiling levers: `3-implementation/substrate.md` §1–§1a.

## Methodology keystones (cross-corpus, load-bearing)

- **Bidirectional ATU test is SOLE arbiter** — gold is candidate-source, not verdict; genre/punctuation/external units NEVER criteria. → [[memories/operational/feedback_external_unit_is_not_atu.md|feedback_external_unit_is_not_atu.md]]
- **Conformance ≠ correctness** — reveal-by-reading on genre-spread sample; validator ticks are proxies, not regressions. → [[memories/operational/feedback_conformance_is_not_correctness.md|feedback_conformance_is_not_correctness.md]]
...
|---|---|
| New binding rule | `1-method/binding-rules-hebrew.md` §"Adding a rule"; retest validated set; no regression = ship. |
