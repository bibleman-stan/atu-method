...
6. **Verify empirically — inspection ≠ verification.** Source-parity (vs original USFM/TF/source) is the correctness gate; index-parity (vs HEAD) is regression-only. Run BOTH tactical (does this break?) and strategic (is this the right design?) audits before any pivot. Any **new mechanism / integration / env-flag / guard structure** triggers the §7.3 audit gate BEFORE code (substrate *consumption* is exempt; building a new integration shape over it is not). Any code-touching edit that **cites canon** ([[1-method/framework.md|framework.md]] / `§2.x` / `binding-rules-*`) requires a fresh in-turn `Read` of cited-range + 30 lines downstream AND a verbatim firewall quote in the proposal. Pre-audit self-check: every cited `file:line` Read this turn, every governing helper observed firing via probe (not inferred), every analogous helper Grep-enumerated. → [[memories/operational/feedback_never_skip_audit_gate.md|feedback_never_skip_audit_gate.md]], [[memories/operational/feedback_conformance_is_not_correctness.md|feedback_conformance_is_not_correctness.md]], [[memories/operational/feedback_canon_citation_requires_verbatim_read.md|feedback_canon_citation_requires_verbatim_read.md]]
