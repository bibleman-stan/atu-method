# Bibleman biblical-reader + ATU orchestrator

Orchestrator orientation for Stan's biblical-reader + ATU methodology work. Lives at `C:\Users\bibleman\CLAUDE.md` (user-home workspace root), auto-loads via Claude Code's directory-walk cascade when the session's cwd is at or below user-home.

**Non-biblical-reader workspaces** (`work/biblical-studies`, `work/rs6310-amridge`, `work/finance`, `work/turkish-study`, `Desktop/claude-consolidation`, etc.) explicitly EXCLUDE this file via `claudeMdExcludes` in their `.claude/settings.json` — so this file genuinely does NOT load in those sessions. **Truly user-wide patterns** (execute-don't-ask, verify-don't-recall, mirror-precedent, etc.) live in `~/.claude/CLAUDE.md` and load in every session.

**In-scope workspaces** (where this file SHOULD load):
- `C:\Users\bibleman\` (user-home — orchestrator primary)
- `C:\Users\bibleman\repos\readers-*` (readers-tanakh, readers-bofm, readers-gnt, readers-gnt-morph, readers-lxx, readers-vulgate)
- `C:\Users\bibleman\repos\atu-method`
- `C:\Users\bibleman\repos\biblical-corpora`
- `C:\Users\bibleman\repos\rev-reader`

If a new non-biblical-reader workspace gets created, **add `claudeMdExcludes` to its `.claude/settings.json`** so this file doesn't leak into it. The `claudeMdExcludes` template:
```json
{ "claudeMdExcludes": ["C:\\Users\\bibleman\\CLAUDE.md"] }
```
...
Unified orchestrator-Claude for Stan's biblical-reader + ATU methodology work at user-home (`C:\Users\bibleman\`). Native access: `repos/`, `Dropbox/`, `vaults-nano/` (PKM read-mostly), `.claude/`. Master-blaster unification arc: `memory/_named_arcs.md`.

---

## STANDING BEHAVIORAL DEFAULTS — ENACT, don't just store

Litmus first; full prose + dated warrants live in the linked memory files (don't re-derive, don't re-ask). The user-wide rules (execute-don't-ask, verify-don't-recall, mirror-precedent, lead-with-WHAT, apologize-once, check-in-cadence) are in `~/.claude/CLAUDE.md` and apply here too — these layer the **biblical-reader-specific** defaults on top.
...
Headlines only; full record + warrants at `memory/_north_star.md` (mandatory orientation read). If something here looks reopenable, it isn't — bring it to Stan with evidence before touching.
...
## Mandatory orientation reads (every wake)

1. **This file** (`~/CLAUDE.md`) — orchestrator orientation
2. **`~/.claude/CLAUDE.md`** — user-wide patterns (already loaded by Claude Code; just be aware of it)
3. **`~/.claude/projects/C--Users-bibleman/memory/MEMORY.md`** — index of operational/project/reference/feedback memories (~36 files)
4. **`~/.claude/projects/C--Users-bibleman/memory/_north_star.md`** — SETTLED decisions (banked / parked / closed-routes / standing tactical). Loaded **every session**, not just on compaction-detect — the settled-decisions layer is never optional.
5. **`git log --oneline -10`** in `~/.claude/` — recent orchestration commits
...
| `~/.claude/MIGRATION_IN_PROGRESS.md` | While migration arc is active |
...
3. Named-arc resume ("continue master-blaster" etc.): pointer in `memory/_named_arcs.md`; archive at `~/.claude/jsonl-archive/<namespace>/<session-id>.jsonl`.
...
| Repo | Role | Live |
|---|---|---|
| `atu-method` | Methodology canon + shared binding-engine | — |
...
| `biblical-corpora` | Shared vendored clones: bhsa/macula-hebrew/greek-new-testament. Reader-specific substrate moved 2026-05-27 to each reader's gitignored `private/substrate/`. Detail: `memory/reference_biblical_studies_folder.md`. | — |

## Pipeline architecture

Mechanical-first per `atu-method/docs/framework.md` §3. **Live state per reader = `atu-method/docs/deployment-status.md` (single source of truth).** All deployed readers run v1.5 binding-rule stage; no v4 (retired 2026-05-22). Never infer deploy state from stale per-repo docs.

Stages: v0 source → v1 treebank clause-atoms → v1.5 binding rules → v2 narrow-task LLM adjudication (optional) → v3 editorial review → deploy. Hebrew F1 85-91% boundary; 5-25% absorption per genre. **Past-ceiling levers:** (a) better *real-gold* substrate [CLOSED for BoFM via manufactured/parser-bootstrap — see Closed routes], (b) cross-corpus convergence-projection (BHSA→alignment→target for Hebrew-source LXX/Vulgate-OT — FIRST mechanical move, not v2 defer), (c) v2 LLM adjudication. Full doctrine + per-corpus state + known limits (BHSA-canon-migration arc): `atu-method/docs/substrate.md` + `_named_arcs.md`.
...
## Editorial discipline + source-text + hooks (pointers)

- Hebrew/Tanakh editorial discipline: `~/repos/readers-tanakh/CLAUDE.md` § Editorial discipline.
- Source-text rules (NEVER modify vendored sources / Hebrew consonants/niqqud/te'amim / word counts; ALWAYS work in `data/text-files/v2/heb/` analog; `PYTHONIOENCODING=utf-8` on Windows): `feedback_workflow.md`.
- Hook system (`check_bash_discipline.py` domain-scopes; full hook blocks heredocs ≥5 lines / unsummarized git-status-diff; override prefixes `# disciplined-allow:` / `# split-justified:` / `# audit-skippable:`): `~/.claude/hooks/`.

## Default decisions (don't surface as menus)

| Decision | Standing answer |
|---|---|
| New binding rule | `atu-method/docs/binding-rules-hebrew.md` §"Adding a rule"; retest validated set; no regression = ship. |
...
Commit + push autonomously on `main` after clean local commits (blanket-auth 2026-05-11): `commit` → if exit 0 → `push origin main` → report. **Confirm BEFORE**: force-push, non-main, agent-bulk-corpus-not-diff-reviewed, first-time-new-private-remote. Atomic-ship doctrine (cross-repo cascade + within-repo rule+regen+HTML+sw.js as ONE), Windows execution hard-rules (HEREDOC commit msgs, `Path(f).as_posix()` for `git show HEAD:path`, `PYTHONIOENCODING=utf-8`, no shell `&` backgrounding, never `--update-baseline`), tree-state `git status --short` pre-commit: `feedback_workflow.md`.

---

## Other directories at this workspace root

- `repos/` — biblical-reader project repos
- `Dropbox/` — datasets (03-Biblical_Studies/, etc.)
- `vaults-nano/` — Obsidian PKM (my_brain/, gospel/) — DO NOT mutate from this Claude; PKM is read-mostly
- `.claude/` — orchestration substrate (CLAUDE.md, settings, hooks, memory, jsonl-archive)
- `Documents/`, `Downloads/`, `Desktop/`, etc. — OS user directories; ignore unless explicitly directed
- `work/` — out-of-scope sibling workspaces (biblical-studies, rs6310-amridge, finance, turkish-study). Each excludes this file via `claudeMdExcludes`.

## Identifying named multi-session arcs

To resume a named conversation arc ("continue master-blaster", "continue X"), read `~/.claude/projects/C--Users-bibleman/memory/_named_arcs.md` — it has JSONL pointers for the relevant archived conversation under `~/.claude/jsonl-archive/`.

---

Detail in `MEMORY.md` (operational/project/feedback) + `atu-method/docs/` (methodology canon).
