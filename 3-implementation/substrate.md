...
Pipeline context: `v0 source → v1 treebank clause-atoms → v1.5 binding rules → v2 narrow-task LLM adjudication (optional) → v3 editorial review → deploy`; stage detail in [`toolset-architecture.md`](toolset-architecture.md). All deployed readers run the v1.5 binding-rule stage; there is no v4 (retired 2026-05-22). Hebrew sits at F1 85–91% on boundaries with 5–25% absorption depending on genre. Live per-reader state is [`../2-evidence/deployment-status.md`](../2-evidence/deployment-status.md) — the single source of truth; never infer deploy state from a stale per-repo doc.
...
- **Lens** — a deterministic function `lens(substrate) → segments` whose
  ...
  the v0 → v1 → v1.5 (→ v2 → v3) pipeline ([`framework.md` §3](<../1-method/framework.md#§3 Architecture — mechanical-first>)); the
...
|---|---|
| **ATU** | Five reader sites named in the orientation repo map at `~/.claude/CLAUDE.md:101-105` (tanakh-reader.com, bomreader.com, gnt-reader.com, lxx-reader.com, vulgate-reader.com). The single-source-of-truth file `2-evidence/deployment-status.md:11-15` records only the first three rows; that file predates the LXX and Vulgate launches and is **stale**. Per standing rule 8 (external-state claims require fresh verification), the actual live-URL state at any given moment must be re-fetched, not inherited from this row. Open follow-up: update [`deployment-status.md`](../2-evidence/deployment-status.md) to add the missing rows and remove the staleness footnote here. |
...
- **Admit genre as an ATU criterion.** A candidate lens (e.g., Korpel
  ...
  ([`framework.md`](../1-method/framework.md) and per-repo memory) holds.
