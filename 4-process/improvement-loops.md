...
**First completed cycle — `findings/F-001-marschall-1ne3.md`.** That vault carried a pilot `findings/` class, whose first entry was the readers-bofm Marschall measurement of deployed 1 Nephi 3 (72 ATU lines against 89 côla; 28% over the 25-syllable standard ceiling; 10% breaking the 35-syllable Law), admitted frozen with provenance pinned to `readers-bofm @ d740af1`.

> **⚠ Corrected 2026-08-08.** `findings/` **no longer exists** in that vault — verified this turn (`ls -d findings` → gone), and `admin/maturation-loops.md` now exists (6,493 bytes, Aug 8 11:25). The wiki session retired the `findings/` class rather than making it permanent, which is the D1 design in [[Pending-Decisions.md]] being executed on their side. **The measurement F-001 therefore has no home until this repo receives it into `2-evidence/`** — that is our half of D1 and it has not been done. This paragraph asserted a live state of another workspace and went stale within hours; it is corrected rather than rewritten so the staleness is visible.
