...
### [2026-08-11] A zero from an uncalibrated probe is not evidence of absence

The promoted rule says *"when a scan says most of the corpus is broken, suspect the scan."* **The mirror image is the one that actually bit, six times in one session: when a scan says nothing is there, suspect the scan.** A false positive announces itself — there is output to inspect and it looks wrong. A false negative produces an empty result that reads exactly like a clean finding, so nothing prompts a second look.

Every instance was an ad-hoc `grep`/`python -c` typed in conversation, and each one's zero went straight into a claim:

| Probe | Reported | Actual | Defect |
|---|---|---|---|
| `grep ' cltype='` across macula-greek | attribute exists nowhere, in any format | **461** in `SBLGNT/nodes/` | case — it is `ClType` |
| `grep -oE "py -3 …"` over hook files | bofm has no stale paths | **2** stale calls | bofm spells it `python3` |
| phrase-span count in `hardy_phrase_index` | 0 spans | **1,109** spans | walked the wrong nesting level |
| absolute-path grep in `settings.json` | 0 machine paths | **6** | over-escaped the pattern |
| tier survey for `3-project` | atu-method missing a tier | complete | it is named `3-implementation` |
| `grep -ci 'hardy'` over tracked files | 3 PDFs leaked | **0** PDFs | matched unrelated filenames |

**The sharpest one is the `settings.json` case, because the evidence was already on screen.** The diff I had just read showed `C:\Users\bibleman\...` in the file. My grep then said zero machine paths, and I printed it. **When a probe contradicts an artifact I have already seen, the probe is wrong — not the artifact.** That should be automatic and it was not.

The `cltype` one cost the most. A single case-insensitive retry reversed a conclusion I had already stated twice, and had I not retried, the standing recommendation would have been to substitute Nestle 1904 for SBLGNT — a text substitution, on a false premise, in the corpus the readers are built on.

**Why §6 did not prevent this.** §6 says a detector is a claim and must assert a known-good and a known-bad *in the script*. That rule is written for checkers under `5-machinery/`, and every one of the six failures above was a throwaway command that will never be a script. **The rule's principle covers them; its mechanism does not reach them.**

**Candidate guard, cheap enough to always do:** before believing a zero, run the same probe against a known-positive. `grep -c 'class=' file` before trusting `grep -c 'cltype=' file` returns nothing. If the control also returns zero, the probe is broken, not the corpus. For vocabulary I have not personally read, probe case-insensitively and for both plausible spellings first, then narrow.

**Two adjacent misses in the same session, same family — asserting without opening:**

- I reported a suite total as *"exactly the gate's expected baseline"* from recall of an earlier session. The baseline file said something else. The number was one `Read` away and I did not run it.
- A `git push` loop printed "pushed" for every repo because the `printf` did not check the push's exit code. One push had failed — rev-reader, which is on `master` with no remote. **Reporting success is a claim too, and mine was generated by the same script that was supposed to verify it.**

And one scope miss with a different shape: `git add -A data books` swept in **45** unrelated backup parse caches plus three modified READMEs. Caught only because I listed the staged set before committing. **`-A` over a directory is not a targeted stage; it is a bet that nothing else in that tree is dirty.**
