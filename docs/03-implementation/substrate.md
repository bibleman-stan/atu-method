...
- **Lens** — a deterministic function `lens(substrate) → segments` whose
  ...
  the v0 → v1 → v1.5 (→ v2 → v3) pipeline ([`framework.md` §3](<../01-normative/framework.md#§3 Architecture — mechanical-first>)); the
...
|---|---|
| **ATU** | Five reader sites named in the orientation repo map at `~/.claude/CLAUDE.md:101-105` (tanakh-reader.com, bomreader.com, gnt-reader.com, lxx-reader.com, vulgate-reader.com). The single-source-of-truth file `docs/05-status/deployment-status.md:11-15` records only the first three rows; that file predates the LXX and Vulgate launches and is **stale**. Per standing rule 8 (external-state claims require fresh verification), the actual live-URL state at any given moment must be re-fetched, not inherited from this row. Open follow-up: update [`deployment-status.md`](../05-status/deployment-status.md) to add the missing rows and remove the staleness footnote here. |
...
- **Admit genre as an ATU criterion.** A candidate lens (e.g., Korpel
  ...
  ([`framework.md`](../01-normative/framework.md) and per-repo memory) holds.
