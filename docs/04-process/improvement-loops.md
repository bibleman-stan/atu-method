...
There is **no substrate loop**: nothing systematically improves parse quality, which [`substrate.md`](../03-implementation/substrate.md) argues is *the* ceiling on output. UD corrections happen in waves when somebody runs one, not as a cycle with a trigger and a measure.
