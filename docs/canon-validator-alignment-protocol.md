...
2. **Closed-list presence.** Every closed-list named in the rule's UD signature or prose appears as a Python constant (uppercase identifier) in the named validator source file. If canon names `GOVERNING_LEMMAS` and the validator source has no such constant, emit `DRIFT`.
