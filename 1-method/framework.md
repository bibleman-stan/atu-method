...
For Hebrew, the validated catalog is **14 binding rules** (B1-B14 with B4 retired); see [`binding-rules-hebrew.md`](../1-method/binding-rules-hebrew.md).
...
3. **Pipeline 5-machinery/scripts**: chapter-parametric `pilot_config.py` + `v1_extract` + `v1_5_apply_bindings` + `v3_compare`
...
**Lexham Discourse Hebrew Bible (LDHB)** is consulted as a calibration reference; the pipeline does not depend on it at runtime. See [`methodology-position.md`](../4-process/methodology-position.md) for the "Lexham-consulted but not utilized" framing.

## §6 Known gaps

Three architectural gaps the v1.5 mechanical layer cannot currently handle:

1. **Sub-clause gapping** — when BHSA puts two ATUs inside one clause-atom via verb-ellipsis in parallel comparatives (Gen 22:17 stars + sand pattern). Requires sub-clause splitter.
...
5. **Defensibility capture.** Every new rule, sub-rule, or merge-override added to the canon MUST carry three elements:
   ...
   These elements live in the per-rule scholarship file (`2-evidence/scholarship/<rule-id>.md`), not in the operational canon entry.
...
4. Document under [`binding-rules-hebrew.md`](../1-method/binding-rules-hebrew.md) (or per-language equivalent)
...
- [`toolset-architecture.md`](../3-implementation/toolset-architecture.md) — pipeline implementation per stage
- [`binding-rules-hebrew.md`](../1-method/binding-rules-hebrew.md) — the 14 validated Hebrew binding rules
- [`apparatus.md`](../3-implementation/apparatus.md) — scope statement
- [`methodology-position.md`](../4-process/methodology-position.md) — LDHB / discourse-grammar relationship
