...
- [`feedback_sense_line_mission.md`](feedback_sense_line_mission.md) — **DEMOTED 2026-08-06** (historical record; do not cite as the mission statement). Its ground — *rhetoric constrains, atomic-thought determines* — lives in [`../1-method/cross-corpus-principles.md`](../1-method/cross-corpus-principles.md) §0.1 + §1.3a and [`feedback_atu_and_rhetorical_lenses_distinct.md`](feedback_atu_and_rhetorical_lenses_distinct.md). Whether [[1-method/framework.md#§1 Purpose|framework.md §1]]'s NOT-list should still exclude the aural + rhetorical lenses is an open Category B question awaiting Stan's ruling.
...
- [`feedback_rule_proposal_gates.md`](feedback_rule_proposal_gates.md) — four-gate checklist run before any rule proposal, closed-list extension, scope claim, or precedence claim. Operationalizes Factors A/B/C plus the bidirectional atomic-thought test as mandatory pre-proposal checks. Catches factor failures before they enter the canon. Cross-references the retraction-log protocol at `atu-method/4-process/retraction-log-protocol.md` (the latency-bounding mechanism). Codified 2026-05-16.
- [`feedback_architecture_must_match_method.md`](feedback_architecture_must_match_method.md) — when scaling a methodology into infrastructure, periodically check that the as-built architecture matches what the method actually claims. The legacy producer-validator architecture drifted from the cognitive-first framework; the 2026-05-18 mechanical-first rewrite realigned them. Periodic re-check applies to future drift.
- [`feedback_cross_corpus_convergence.md`](feedback_cross_corpus_convergence.md) — Stan's program thesis: ATUs reveal common cross-linguistic patterns because thought/language share structure; the multi-corpus build (Tanakh→GNT→LXX→Vulgate→BoM) should EMPIRICALLY demonstrate it. Greek binding rules mirror Hebrew B1–B14 (ὅτι↔ki/B11, ὅς↔ʾăšer/B3, vocative/B1, infinitive/B10, participle/B13). Discipline: look back to built corpora and port the proven mechanism, BUT earn the convergence — re-derive each ported rule independently on its own corpus (bidirectional test + proposal gates), don't assume it; counterexamples are signal; scope-stake nothing prematurely. Includes the three-layer model (universal ATU / language rule-catalog / genre-as-sampler).
- [`feedback_atu_as_textfabric_node_layer.md`](feedback_atu_as_textfabric_node_layer.md) — scholarly-positioning framing (Stan-flagged 2026-05-20): an ATU is a Text-Fabric node-layer added over the grammatical layers — a lasso grouping slots by COMPLETE THOUGHT rather than by clause; grammar = scaffold, ATU = layer above ("grammar constrains, thought determines"). Locates ATU in an established paradigm (Roorda/ETCBC), gives a precise reproducible definition, pre-empts "isn't this just clause-splitting?". Caveat: TF is agnostic infrastructure — its design motivation (overlapping hierarchies = meaning ≠ versification) confirms the premise, not the ATU's privileged status (that rests on the atomic-thought test).
