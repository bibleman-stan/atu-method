...
**How to apply:**
1. **Inventory data sources at task start.** For GNT work the engine currently sources the *thin* `sblgnt-lowfat`; the *rich* Macula Greek lowfat (`readers-gnt/research/macula-greek/SBLGNT/lowfat/`, helper 5-machinery/scripts `macula_clauses.py`/`macula_predication.py`/`macula_valency.py`/`macula_wordgroups.py`) carries clause role/rule/frame/referent. Lexham Discourse GNT (LDGNT) lives at `Dropbox/03-Biblical_Studies/Greek/discourse/Lexham-Discourse-GNT`. See [[reference_corpus_pipeline_map]] / the resource catalog.
