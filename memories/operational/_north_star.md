...
- **BoFM forward = three-lever framework over baseline Stanza** (none is the closed parser-training route):
  1. **Binding-rule additions** in `scripts/bofm_generate.py` — `§2.2`-style rules that consume UD features (e.g. parallel-subordinator-stack rule shipped 2026-06-01 `9101ea9`). Substrate-first permanent fix for the *reader*; each rule subsumes a class of override workarounds. **Highest leverage when a class has a clean structural signature — try this first.**
...
- The doctrine canon stays at `~/repos/atu-method/docs/substrate.md`; this file is the *decision*
