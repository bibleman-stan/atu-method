...
This was the same failure mode named in [[feedback_mechanical_first_for_own_review]] ("structurally impossible / defer to v2" said about features Macula tags mechanically), now applied at the time-estimate layer. The substrate doctrine ([[atu-method/docs/substrate.md]] §1) says: "The ATU framework is a Container, not an Originator." Time estimates inherit this — when you're CONSUMING a substrate, hours; when you're ORIGINATING one, weeks. Confusing the two is the time-tax this memory exists to prevent.
