...
description: "Never assert deployment/architecture state from docs, memory, or an agent's claim — verify against git log of the live dir + atu-method/docs/deployment-status.md"
metadata: 
  node_type: memory
  type: feedback
...
**Never state what is deployed/live, or a corpus's architecture, from a per-repo `CLAUDE.md`/README, a memory, or an agent's relayed claim.** Those drift stale. **Ground truth = (1) `atu-method/docs/deployment-status.md` (the single source of truth), and (2) the `git log` of the live corpus directory** (`git log -- <live-path>`). Verify BEFORE asserting.
