...
- [`canon-validator-alignment-protocol.md`](canon-validator-alignment-protocol.md) — **Structural alignment check specification.** What per-repo `validators/canon/check_canon_alignment.py` 5-machinery/scripts must implement: validator file presence, closed-list presence in source, UD signature field consistency, multi-valued field branches. Verdict taxonomy: ALIGNED / NO_IMPL / DRIFT / PARTIAL / EDITORIAL_ACK. Read when implementing the per-repo script or running the alignment check.
