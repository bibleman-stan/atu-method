...
**Layer assignment criterion.** A prohibition lives in **Layer 1** when both its principle and its operationalization are language-universal — the canonical Layer 1 prohibitions above (line-final CCONJ seeking next member, DET seeking head, AUX seeking V, etc.) apply identically across all languages and require no corpus-specific closed list. A prohibition lives in **Layer 3** when its principle may be universal but its operationalization requires a corpus-specific closed list: complement integrity is a universal grammatical principle, but *which verb classes* require complement-integrity merging differs by language and is therefore captured in per-corpus §5; formula integrity is a universal principle, but the specific multi-word frames (BoFM *And it came to pass*, Tanakh *wayyiqtol* formulae, GNT *καὶ ἐγένετο*) are corpus-specific. Boundary cases — rules whose prohibition has both a universal core and a corpus-specific extension — are documented via rule-template multi-valued Layer fields rather than forced into single-layer assignment. (Defensibility-capture per §7.4; clarifies an assignment criterion that was previously implicit. Lesson surfaced 2026-05-14 from outside-eye audit feedback.)
...
The change protocol (§7.1 Authority through §7.8 Proposed-rule adoption protocol) has been extracted to its own canonical document: [`change-protocol.md`](change-protocol.md). Per-corpus canons reference it via the same stable section IDs (`§7.1`, `§7.2`, etc.). This split keeps the framework specification (Parts I–III) and the change protocol (Part IV) independently versionable and avoids the per-repo §7 inlining problem the protocol's own §7.1 forbids.
